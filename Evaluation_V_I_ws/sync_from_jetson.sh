...
# ── 3. YOLO training dataset (collect_yolo_dataset.py output) ─────────────────
...
echo ""
echo "  Eval dataset:"
find "${LOCAL_EVAL}"     -name "*.jpg" 2>/dev/null | wc -l | xargs -I{} echo "    {} images → eval_dataset/"
echo "  Pipeline captures:"
find "${LOCAL_CAPTURES}" -name "*.jpg" 2>/dev/null | wc -l | xargs -I{} echo "    {} images → captures_from_jetson/"
echo "  YOLO training data:"
find "${LOCAL_YOLO}"     -name "*.jpg" 2>/dev/null | wc -l | xargs -I{} echo "    {} images → yolo_dataset/"
echo "══════════════════════════════════════════════════════"
echo ""
